...
export default function Home() {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        toast.success("繧ｷ繧ｹ繝・Β繧ｪ繝ｳ繝ｩ繧､繝ｳ: 迴ｾ蝨ｨ蝨ｰ繧貞叙蠕励＠縺ｾ縺励◆");
      },
      () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSearchQuery("");
        setSearchResults([]);
        toast.success(`蝣ｴ謇繧定ｿｽ蜉縺励∪縺励◆: ${newLocation.name}`);
      } else {
        toast.error("蝣ｴ謇縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縺ｧ縺励◆");
      }
    } catch (error) {
      console.error("Search error:", error);
      toast.error("讀懃ｴ｢荳ｭ縺ｫ繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆");
    } finally {
  ...
  const addLocation = async () => {
    ...
    setMarkedLocations(prev => [...prev, newLocation]);
    toast.success("繧ｿ繝ｼ繧ｲ繝・ヨ蠎ｧ讓吶ｒ險倬鹸縺励∪縺励◆");
  };

  const removeLocation = (id: string) => {
    setMarkedLocations(prev => prev.filter(loc => loc.id !== id));
    toast.info("繧ｿ繝ｼ繧ｲ繝・ヨ蠎ｧ讓吶ｒ蜑企勁縺励∪縺励◆");
  };
  ...
  const handleMapClick = async (lat: number, lng: number) => {
    ...
    setMarkedLocations(prev => [...prev, newLocation]);
    toast.success("謖・ｮ壼ｺｧ讓吶ｒ繝槭・繧ｯ縺励∪縺励◆");
  };
  ...
  const saveTargetDetails = () => {
    ...
    toast.success("繧ｿ繝ｼ繧ｲ繝・ヨ諠・ｱ繧呈峩譁ｰ縺励∪縺励◆");
  ...
  const exportData = (format: 'json' | 'csv') => {
    ...
    toast.success(`${format.toUpperCase()}蠖｢蠑上〒繧ｨ繧ｯ繧ｹ繝昴・繝医＠縺ｾ縺励◆`);
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (Array.isArray(newLocations) && newLocations.length > 0) {
          ...
          toast.success(`${validLocations.length}莉ｶ縺ｮ繧ｿ繝ｼ繧ｲ繝・ヨ繧偵う繝ｳ繝昴・繝医＠縺ｾ縺励◆`);
        } else {
          toast.error("譛牙柑縺ｪ繝・・繧ｿ縺瑚ｦ九▽縺九ｊ縺ｾ縺帙ｓ縺ｧ縺励◆");
        }
      } catch (error) {
        console.error("Import error:", error);
        toast.error("繝輔ぃ繧､繝ｫ縺ｮ隱ｭ縺ｿ霎ｼ縺ｿ縺ｫ螟ｱ謨励＠縺ｾ縺励◆");
      }
  ...
  const toggleTracking = usePersistFn(() => {
    if (isTracking) {
      ...
      setIsTracking(false);
      toast.info("霑ｽ霍｡繝｢繝ｼ繝・ 蛛懈ｭ｢");
    } else {
      // Start tracking
      if (!navigator.geolocation) {
        toast.error("縺薙・繝悶Λ繧ｦ繧ｶ縺ｯ菴咲ｽｮ諠・ｱ繧偵し繝昴・繝医＠縺ｦ縺・∪縺帙ｓ");
        return;
      }
      ...
      watchIdRef.current = navigator.geolocation.watchPosition(
        ...
        (error) => {
          console.error(error);
          toast.error("菴咲ｽｮ諠・ｱ縺ｮ霑ｽ霍｡荳ｭ縺ｫ繧ｨ繝ｩ繝ｼ縺檎匱逕溘＠縺ｾ縺励◆");
        },
        {
      ...
      setIsTracking(true);
      toast.success("霑ｽ霍｡繝｢繝ｼ繝・ 襍ｷ蜍穂ｸｭ");
    }
  });
              ...
              <div className="flex gap-1">
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent><p>JSON繧ｨ繧ｯ繧ｹ繝昴・繝・/p></TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent><p>CSV繧ｨ繧ｯ繧ｹ繝昴・繝・/p></TooltipContent>
                  </Tooltip>
                  ...
                  <Tooltip>
                    ...
                    <TooltipContent><p>繧､繝ｳ繝昴・繝・(JSON/CSV)</p></TooltipContent>
                  </Tooltip>
                  <input 
                    type="file" 
                    ref={fileInputRef} 
              ...
              filteredLocations.map((loc, index) => (
                    <div 
                      ...
                      onDoubleClick={() => {
                        ...
                        toast.info(`繧ｿ繝ｼ繧ｲ繝・ヨ縺ｸ遘ｻ蜍・ ${loc.name || "Unknown Location"}`);
                      }}
                    >
                  ...
                  onClick={() => {
                    ...
                    navigator.geolocation.getCurrentPosition(
                      (position) => {
                        ...
                        setCurrentLocation(pos);
                        toast.success("迴ｾ蝨ｨ蝨ｰ縺ｸ遘ｻ蜍輔＠縺ｾ縺励◆");
                      },
                      () => {
                        // Fallback to last known location if GPS fails
                        setCurrentLocation({...currentLocation});
                        toast.info("迴ｾ蝨ｨ蝨ｰ縺ｸ遘ｻ蜍輔＠縺ｾ縺励◆");
                      }
                    );
                  }}
                >
